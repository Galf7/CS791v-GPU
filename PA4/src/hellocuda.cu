...
int main() {
  ...
  cudaMallocManaged( (void**) &dev_d, (matSize*matSize) * sizeof(int));
  cudaMallocManaged( (void**) &dev_e, (matSize*matSize) * sizeof(int));
  ...
  for(int iter = 0; iter<matSize;iter++){
	  ...
	  cudaMallocManaged( (void**) &(dev_d[iter]), (matSize)*sizeof(int));
	  cudaMallocManaged( (void**) &(dev_e[iter]), (matSize)*sizeof(int));
  	  for(int cur = 0; cur<matSize;cur++){
  		dev_a[iter][cur] = iter*cur;
  		dev_b[iter][cur] = iter*cur;
  		dev_d[iter][cur] = iter*cur;
  		dev_e[iter][cur] = iter*cur;
  		dev_c[iter][cur] = 0;
  	  }
    }
  //std::cout << "3" << std::endl;
  ...
  if(sequential > 0){
	  ...
	  for(int x = 0; x<matSize; x++){
		  for(int y=0; y<matSize; y++){
			  dev_c[x][y] = (dev_a[x][y] * dev_b[y][x]) + (dev_d[x][y] * dev_e[y][x]);
		  }
	  }
	  //std::cout << "6" << std::endl;
  ...
  */
  ...
  for (int i = 0; i < matSize; ++i) {
	  for(int j = 0; j < matSize; j++){
		  if (dev_c[i][j] != (dev_a[i][j] * dev_b[j][i]) + (dev_d[i][j] * dev_e[j][i])) {
